...
- Added RFC 9562 UUIDv7 generation, prefixed IDs, and cross-ledger resource references for independently allocated records
...
- Fixed `MigrationLock` creating `.ledger/migrations/write.lock` with executable
  permissions on POSIX; lock files now use private `0600` permissions, stale
  diagnostics are replaced only after lock acquisition, and failed acquisition
